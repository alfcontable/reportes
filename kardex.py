...
--------------- EMPIEZA LA UNION 
    ...
    costo_unit
		...
		from 
		(
			select sm.id,sm.date_expected  AT TIME ZONE 'UTC' as date_expected,sm.product_id,sm.name,sm.company_id,sm.product_qty as u_entrada,(sm.product_qty * 0)u_salida,
		...
		from
		   (
				select sm.id,sm.date_expected AT TIME ZONE 'UTC' as date_expected,sm.product_id,sm.name,sm.company_id,(sm.product_qty * 0) as u_entrada,
